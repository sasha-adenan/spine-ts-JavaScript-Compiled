...
var spine = (() => {
  ...
  var DarkTintGeometry = class extends import_core3.Geometry {
    /**
    ...
    constructor(_static = false) {
      super();
      const verticesBuffer = new import_core3.Buffer(void 0);
      const uvsBuffer = new import_core3.Buffer(void 0, true);
      const indexBuffer = new import_core3.Buffer(void 0, true, true);
      this.addAttribute("aVertexPosition", verticesBuffer, 2, false, import_core3.TYPES.FLOAT);
      this.addAttribute("aTextureCoord", uvsBuffer, 2, false, import_core3.TYPES.FLOAT);
...
`;
  var DarkTintMaterial = class extends import_core4.Shader {
    uvMatrix;
    batchable;
    pluginName;
    ...
    constructor(texture) {
      const uniforms = {
        uSampler: texture ?? import_core4.Texture.EMPTY,
        alpha: 1,
        uTextureMatrix: import_core4.Matrix.IDENTITY,
      ...
      super(import_core4.Program.from(vertex, fragment), uniforms);
      this._colorDirty = false;
      this.uvMatrix = new import_core4.TextureMatrix(uniforms.uSampler);
      this.batchable = true;
      this.pluginName = options.pluginName;
      this._tintColor = new import_core4.Color(options.tint);
      this._darkTintColor = new import_core4.Color(options.darkTint);
    ...
    update() {
      if (this._colorDirty) {
        this._colorDirty = false;
        import_core4.Color.shared.setValue(this._tintColor).premultiply(this._alpha, true).toArray(this.uniforms.uColor);
        import_core4.Color.shared.setValue(this._darkTintColor).premultiply(this._alpha, true).premultiply(1, false).toArray(this.uniforms.uDarkColor);
      }
  ...
  var Spine = class _Spine extends import_display.Container {
    ...
    set autoUpdate(value) {
      if (value && !this._autoUpdate) {
        import_core6.Ticker.shared.add(this.internalUpdate, this);
      } else if (!value && this._autoUpdate) {
        import_core6.Ticker.shared.remove(this.internalUpdate, this);
      }
    ...
    _boundsPoint = new import_core5.Point();
    _boundsSpineID = -1;
    ...
    constructor(options, oldOptions) {
      ...
      if (options?.darkTint !== void 0 || oldOptions?.slotMeshFactory === void 0) {
    ...
    */
    ...
    internalUpdate(_deltaFrame, deltaSeconds) {
      ...
      const delta = deltaSeconds ?? import_core6.Ticker.shared.deltaMS / 1e3;
    ...
    updateAndSetPixiMask(slot, last) {
      ...
      if (currentClippingSlot && slotObject) {
        ...
        if (!currentClippingSlot.maskComputed) {
          ...
          mask.clear().lineStyle(0).beginFill(0).drawPolygon(vertices).endFill();
        }
    ...
    */
      ...
      for (let i = 0, n = drawOrder.length, slotObjectsCounter = 0; i < n; i++) {
        ...
        if (texture != null) {
          ...
          if (_Spine.clipper.isClipping()) {
            _Spine.clipper.clipTriangles(this.verticesCache, triangles, triangles.length, uvs, this.lightColor, this.darkColor, useDarkColor);
    ...
    static from(paramOne, atlasAssetName, options) {
      if (typeof paramOne === "string") {
        return _Spine.oldFrom(paramOne, atlasAssetName, options);
      }
      ...
      if (!skeletonData) {
        const skeletonAsset = import_assets.Assets.get(skeleton);
        const atlasAsset = import_assets.Assets.get(atlas);
      ...
      return new _Spine({ skeletonData, darkTint, autoUpdate, boundsProvider });
    }
  ...
  var maskPool = new Pool(() => new import_graphics.Graphics());
  Skeleton.yDown = true;
  ...
  var spineTextureAtlasLoader = {
    ...
    resolver: {
      test: (value) => (0, import_assets3.checkExtension)(value, ".atlas"),
      parse: (value) => {
        const split = value.split(".");
        return {
          resolution: parseFloat(import_core7.settings.RETINA_PREFIX?.exec(value)?.[1] ?? "1"),
    ...
    loader: {
      ...
      async parse(asset, options, loader) {
        ...
        let basePath = import_core7.utils.path.dirname(options.src);
        if (basePath && basePath.lastIndexOf("/") !== basePath.length - 1) {
          basePath += "/";
        }
        const retval = new TextureAtlas(asset);
        if (metadata.images instanceof import_core7.BaseTexture || typeof metadata.images === "string") {
        ...
        for (const page of retval.pages) {
          ...
          if (providedPage instanceof import_core7.BaseTexture) {
            page.setTexture(SpineTexture.from(providedPage));
          } else {
            const url = providedPage ?? import_core7.utils.path.normalize([...basePath.split(import_core7.utils.path.sep), pageName].join(import_core7.utils.path.sep));
            const assetsToLoadIn = { src: (0, import_assets2.copySearchParams)(url, options.src), data: { ...metadata.imageMetadata, ...{ alphaMode: page.pma ? import_core7.ALPHA_MODES.PMA : import_core7.ALPHA_MODES.UNPACK } } };
  ...
  import_core7.extensions.add(spineTextureAtlasLoader);
  ...
  var spineLoaderExtension = {
    extension: import_core8.ExtensionType.Asset,
    loader: {
      name: loaderName2,
      extension: {
        type: import_core8.ExtensionType.LoadParser,
      ...
      async load(url) {
        const response = await import_core8.settings.ADAPTER.fetch(url);
  ...
  import_core8.extensions.add(spineLoaderExtension);

  // spine-pixi-v7/src/darkTintMesh/DarkTintBatchGeom.ts
  var import_core9 = __require("@pixi/core");
  var DarkTintBatchGeometry = class extends import_core9.Geometry {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _buffer;
    // eslint-disable-next-line @typescript-eslint/naming-convention
    _indexBuffer;
    ...
    constructor(_static = false) {
      super();
      this._buffer = new import_core9.Buffer(void 0, _static, false);
      this._indexBuffer = new import_core9.Buffer(void 0, _static, true);
      this.addAttribute("aVertexPosition", this._buffer, 2, false, import_core9.TYPES.FLOAT).addAttribute("aTextureCoord", this._buffer, 2, false, import_core9.TYPES.FLOAT).addAttribute("aColor", this._buffer, 4, true, import_core9.TYPES.UNSIGNED_BYTE).addAttribute("aDarkColor", this._buffer, 4, true, import_core9.TYPES.UNSIGNED_BYTE).addAttribute("aTextureId", this._buffer, 1, true, import_core9.TYPES.FLOAT).addIndex(this._indexBuffer);
    }
  };

  // spine-pixi-v7/src/darkTintMesh/DarkTintRenderer.ts
  var import_core10 = __require("@pixi/core");
  var vertex2 = `
...
`;
  var DarkTintRenderer = class extends import_core10.BatchRenderer {
    static extension = {
      name: "darkTintBatch",
      type: import_core10.ExtensionType.RendererPlugin
    };
    constructor(renderer) {
      super(renderer);
      this.shaderGenerator = new import_core10.BatchShaderGenerator(vertex2, fragment2);
    ...
    packInterleavedGeometry(element, attributeBuffer, indexBuffer, aIndex, iIndex) {
      ...
      const argb = import_core10.Color.shared.setValue(element._tintRGB).toPremultiplied(worldAlpha, true);
      const darkargb = import_core10.Color.shared.setValue(element._darkTintRGB).premultiply(worldAlpha, true).toPremultiplied(1, false);
  ...
  import_core10.extensions.add(DarkTintRenderer);
