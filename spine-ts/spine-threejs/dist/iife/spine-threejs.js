...
var spine = (() => {
  ...
  __export(index_exports, {
    ...
    SkeletonMeshMaterial: () => SkeletonMeshMaterial,
    Skin: () => Skin,
    SkinEntry: () => SkinEntry,
    Slot: () => Slot,
    SlotData: () => SlotData,
  ...
  // spine-threejs/src/require-shim.ts
  ...
  // spine-threejs/src/ThreeJsTexture.ts
  ...
  var ThreeJsTexture = class _ThreeJsTexture extends Texture {
    ...
    setFilters(minFilter, magFilter) {
      this.texture.minFilter = _ThreeJsTexture.toThreeJsMinificationTextureFilter(minFilter);
      this.texture.magFilter = _ThreeJsTexture.toThreeJsMagnificationTextureFilter(magFilter);
    }
    setWraps(uWrap, vWrap) {
    ...
    static toThreeJsMinificationTextureFilter(filter) {
      if (filter === 9729 /* Linear */) return THREE.LinearFilter;
      else if (filter === 9987 /* MipMap */) return THREE.LinearMipMapLinearFilter;
      else if (filter === 9985 /* MipMapLinearNearest */) return THREE.LinearMipMapNearestFilter;
      else if (filter === 9986 /* MipMapNearestLinear */) return THREE.NearestMipMapLinearFilter;
      else if (filter === 9984 /* MipMapNearestNearest */) return THREE.NearestMipMapNearestFilter;
      else if (filter === 9728 /* Nearest */) return THREE.NearestFilter;
      else throw new Error("Unknown texture filter: " + filter);
    }
    static toThreeJsMagnificationTextureFilter(filter) {
      if (filter === 9729 /* Linear */) return THREE.LinearFilter;
      else if (filter === 9987 /* MipMap */) return THREE.LinearFilter;
      else if (filter === 9985 /* MipMapLinearNearest */) return THREE.NearestFilter;
      else if (filter === 9986 /* MipMapNearestLinear */) return THREE.LinearFilter;
      else if (filter === 9984 /* MipMapNearestNearest */) return THREE.NearestFilter;
      else if (filter === 9728 /* Nearest */) return THREE.NearestFilter;
      else throw new Error("Unknown texture filter: " + filter);
    }
    static toThreeJsTextureWrap(wrap) {
      if (wrap === 33071 /* ClampToEdge */) return THREE.ClampToEdgeWrapping;
      else if (wrap === 33648 /* MirroredRepeat */) return THREE.MirroredRepeatWrapping;
      else if (wrap === 10497 /* Repeat */) return THREE.RepeatWrapping;
      else throw new Error("Unknown texture wrap: " + wrap);
    }
    static fist = true;
    static toThreeJsBlending(blend) {
      if (blend === 0 /* Normal */) return { blending: THREE.NormalBlending };
      else if (blend === 1 /* Additive */) return { blending: THREE.AdditiveBlending };
      else if (blend === 2 /* Multiply */) return {
  ...
  // spine-threejs/src/AssetManager.ts
  ...
  // spine-threejs/src/MeshBatcher.ts
  var THREE3 = __toESM(__require("three"), 1);

  // spine-threejs/src/SkeletonMesh.ts
  var THREE2 = __toESM(__require("three"), 1);
  var SkeletonMesh = class _SkeletonMesh extends THREE2.Object3D {
    ...
    constructor(skeletonDataOrConfiguration, materialCustomizer = () => {
      ...
      if (!("skeletonData" in skeletonDataOrConfiguration)) {
        const materialFactory = () => {
          const parameters = { ..._SkeletonMesh.DEFAULT_MATERIAL_PARAMETERS };
          materialCustomizer(parameters);
          return new THREE2.MeshBasicMaterial(parameters);
        };
      ...
      this.materialFactory = skeletonDataOrConfiguration.materialFactory ?? (() => new THREE2.MeshBasicMaterial(_SkeletonMesh.DEFAULT_MATERIAL_PARAMETERS));
    ...
    nextBatch() {
      if (this.batches.length == this.nextBatchIndex) {
        let batch2 = new MeshBatcher(MeshBatcher.MAX_VERTICES, this.materialFactory, this.twoColorTint);
        batch2.castShadow = this._castShadow;
        batch2.receiveShadow = this._receiveShadow;
        this.add(batch2);
  ...
  // spine-threejs/src/MeshBatcher.ts
  var MeshBatcher = class _MeshBatcher extends THREE3.Mesh {
    constructor(maxVertices = _MeshBatcher.MAX_VERTICES, materialFactory, twoColorTint = true) {
      super();
      this.materialFactory = materialFactory;
      this.twoColorTint = twoColorTint;
      if (maxVertices > _MeshBatcher.MAX_VERTICES) throw new Error("Can't have more than 10920 triangles per batch: " + maxVertices);
      if (twoColorTint) {
        this.vertexSize += 3;
      }
      ...
      const vertexBuffer = new THREE3.InterleavedBuffer(this.vertices, this.vertexSize);
      this.vertexBuffer = vertexBuffer;
      this.vertexBuffer.usage = WebGLRenderingContext.DYNAMIC_DRAW;
      const geo = new THREE3.BufferGeometry();
      geo.setAttribute("position", new THREE3.InterleavedBufferAttribute(vertexBuffer, 3, 0, false));
      geo.setAttribute("color", new THREE3.InterleavedBufferAttribute(vertexBuffer, 4, 3, false));
      geo.setAttribute("uv", new THREE3.InterleavedBufferAttribute(vertexBuffer, 2, 7, false));
      if (twoColorTint)
        geo.setAttribute("darkcolor", new THREE3.InterleavedBufferAttribute(vertexBuffer, 3, 9, false));
      const normalBuffer = new THREE3.BufferAttribute(normals, 3);
      normalBuffer.usage = WebGLRenderingContext.STATIC_DRAW;
      geo.setAttribute("normal", normalBuffer);
      const indexBuffer = new THREE3.BufferAttribute(this.indices, 1);
    ...
    dispose() {
      ...
      if (this.material instanceof THREE3.Material)
    ...
    clear() {
      ...
      if (this.material instanceof THREE3.Material) {
        ...
        meshMaterial.blending = THREE3.NormalBlending;
    ...
    findMaterialGroup(slotTexture, slotBlendMode) {
      ...
      if (Array.isArray(this.material)) {
        for (let i = 0; i < this.material.length; i++) {
          ...
          if (!meshMaterial2.map) {
            updateMeshMaterial(meshMaterial2, slotTexture, blendingObject);
            return i;
          }
          if (meshMaterial2.map === slotTexture && blendingObject.blending === meshMaterial2.blending && (blendingObject.blendSrc === void 0 || blendingObject.blendSrc === meshMaterial2.blendSrc) && (blendingObject.blendDst === void 0 || blendingObject.blendDst === meshMaterial2.blendDst) && (blendingObject.blendSrcAlpha === void 0 || blendingObject.blendSrcAlpha === meshMaterial2.blendSrcAlpha) && (blendingObject.blendDstAlpha === void 0 || blendingObject.blendDstAlpha === meshMaterial2.blendDstAlpha)) {
            return i;
          }
        }
  ...
  var spineOnBeforeCompile = (shader) => {
    let code;
    shader.vertexShader = `
  ...
  var SkeletonMeshMaterial = class extends THREE3.ShaderMaterial {
    get map() {
